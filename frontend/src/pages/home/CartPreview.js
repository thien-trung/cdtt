...
class CartPreview extends Component {
    ...
    getShoppingCartList() {
        axios.get(`${Api}/product/cart-list/`, {
            ...
            let localCartList = localStorage.getItem('cartList') || null;
    ...
    calcTotal() {
        ...
        this.state.cartList.forEach(item => {
            subtotal += (item.stock?.product?.price || 0) * (item.quantity || 0);
        });
                                    ...
                                    {this.state.cartList.map((item) => (
                                        <div key={item.id} className="product-widget">
                                            <div className="product-img">
                                                <img 
                                                    src={item.stock?.product?.photo ? require(`../../assets/img/${item.stock.product.photo}`) : require('../../assets/img/hotdeal.png')} 
                                                    alt={item.stock?.product?.name || 'Unnamed Product'} 
                                                />
                                            </div>
                                            <div className="product-body">
                                                <h3 className="product-name">
                                                    <Link to={`/products/${item.stock?.product?.id}`}>
                                                        {item.stock?.product?.name || 'Unnamed Product'}
                                                    </Link>
                                                </h3>
                                                <h4 className="product-price">
                                                    <span className="qty">{item.quantity}x</span>${item.stock?.product?.price || 'N/A'}
                                                </h4>
                                            </div>
                                            <button id={item.id} className="delete" onClick={this.handleDelete}>
